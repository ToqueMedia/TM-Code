...
After the GIS callback exchanges the Google credential for Firebase tokens via `POST /api/auth/proxy/google`, you **MUST** execute these four calls IN ORDER:
...
`/api/auth/proxy/google` is a **same-origin** path. Without `vite.config.ts` proxying `/api` to the backend, the request hits port 5173 and returns **404**. See `auth-proxy-gip` skill, hard rule #8 — the snippet is one line and **MUST** be present in `vite.config.ts` before this hook can possibly work.
...
This strips the \`Referer\` header on the image fetch, and Google's CDN serves the photo. Applies to ALL profile-photo URLs returned by Identity Toolkit's \`signInWithIdp\` for \`google.com\` provider — never skip the attribute even when it "works on my machine" (cached by previous fetches).
...
> **Monorepo note:** `.env` is at the project root. If your Vite app lives in `client/` (or any subdirectory), Vite won't find `VITE_GOOGLE_CLIENT_ID` by default and the GIS button will silently fail to render. See `auth-proxy-gip` skill, "Frontend (Vite) — `VITE_*` vars in monorepo layouts" for the `envDir` fix.
