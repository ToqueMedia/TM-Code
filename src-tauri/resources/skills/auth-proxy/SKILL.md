...
### CRITICAL — Client-owned columns: leave `null` on insert; never auto-fill server-side

**Detection rule (run BEFORE writing the auth-sync insert)**:

1. Read the schema file (`schema.ts` / `prisma/schema.prisma` / wherever).
2. Look for columns on the `users` table whose values are decided by the CLIENT, not by the JWT or by the server. Common shapes:
   - `salt` (master-password PBKDF2 salt — generated by the user's browser via `crypto.getRandomValues`)
   - `master_password_verification` / `verification_token` (ciphertext the client encrypts after setting a password)
   - `totp_secret` (in projects where the client generates it during enrollment, not the server)
   - any `*_at_rest_*` or `e2ee_*` columns
3. **If ANY of these exist → those columns MUST be `null` on the insert path of `/api/auth/sync`.**
4. If NONE exist (vanilla user record: uid, email, name, avatar, role, timestamps) → this rule is a no-op, ignore it.

**Why this matters** — the frontend gates its "first-setup" flow on the column being `null`:

```ts
// UnlockModal (or equivalent) — client-side
const isSetup = !user.salt              // ← gates the "create master password" UI
if (isSetup) {
  const salt = crypto.getRandomValues(new Uint8Array(32))   // CLIENT generates
  const key = await deriveKey(masterPassword, salt)
  // ... store ciphertext-encoded verification using `key`
  await fetch('/api/auth/update-salt', {                    // CLIENT POSTs to server
    method: 'POST',
    body: JSON.stringify({ salt, verificationToken }),
  })
}
```

If the server auto-fills the column on user creation, `isSetup === false` on first load, the frontend asks for an EXISTING master password the user never set, and the user is locked out of the account immediately after sign-up.

**Anti-pattern** — ANY of these in the `/api/auth/sync` insert handler:
- `salt: randomBytes(32).toString('base64')`
- `salt: crypto.randomUUID()`
- `verification_token: someDefault`
- `totp_secret: speakeasy.generateSecret().base32`

All wrong. Server has no business filling client-owned columns.

**The correct flow** is three explicit steps, server-side written ONCE per user:

1. **`/api/auth/sync` insert** — uid, email, name, avatar_url, role; client-owned columns left `null`.
2. **`/api/auth/update-salt`** (or per-project equivalent) — client POSTs the salt + verification ciphertext when the user sets their master password. Server stores both.
3. **`/api/auth/me`** — returns the user row with salt populated; frontend reads `user.salt` to decide which UI branch (setup vs unlock).

**Override-gambit pre-empt** — *"the column exists, so it needs a value"* is wrong. Nullable columns are deferred values the client owns. Don't fill columns just because they're there.
...
1. The auth-proxy is a thin REST forwarder. It talks to the platform's auth API directly using the public client key (`VITE_TM_AUTH_KEY` / `TM_AUTH_KEY`). The data layer (where user records persist) is a separate concern and uses Drizzle + libSQL per the `publish-backend` skill — not a contradiction; two surfaces with different needs.
