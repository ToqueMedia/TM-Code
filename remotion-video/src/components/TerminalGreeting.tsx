...
import { PROJECT } from '../data/agentScript'

export interface TerminalGreetingProps {
...
// PROJECT.tip already carries the "tip: " prefix — strip it so the styled
// purple "tip:" label is not duplicated.
const TIP_TEXT = PROJECT.tip.replace(/^tip:\s*/, '')

/**
 * Terminal-mode welcome banner (ASCII logo, project line, branch, tip).
 * Each block fades in + rises 6px, staggered 5 frames apart.
 */
export const TerminalGreeting: React.FC<TerminalGreetingProps> = ({ startFrame = 0 }) => {
  ...
  return (
    <div style={{ fontFamily: tokens.fontFamily.mono }}>
      ...
      >
        <span style={{ color: tokens.colors.accent.green }}>{'→ ' + PROJECT.branch}</span>
      ...
      >
        ...
        <span style={{ color: tokens.colors.text.muted }}>{TIP_TEXT}</span>
      </div>
    </div>
  )
}
