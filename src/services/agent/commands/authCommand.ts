...
function buildPrompt(
  ...
  return [
    ...
    `4. Persist users via the platform data layer — Drizzle + libSQL, exactly as the Publishing section of your system prompt and the \`publish-backend\` skill describe. The auth-proxy is concerned with sign-in flow only; the user record (uid PK, email unique, name, avatarUrl, role, createdAt, updatedAt) lives in the project's OWN \`users\` table: local SQLite file (\`DATABASE_URL=file:./dev.db\`) in dev, TM Code Database (\`TMDB_URL\`/\`TMDB_TOKEN\` injected by Publish/deploy into Cloud Run) in prod. Do NOT use \`firebase-admin\` for persistence — the project has no Admin credentials by design, and \`admin.initializeApp()\` without them crashes the sync route at runtime with "Could not load the default credentials". Custom columns are nullable or have defaults — sync runs from JWT data on first sign-in.`,
    ``,
