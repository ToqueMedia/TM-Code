...
export function registerProvisionTools(ctx: ToolRegistrationContext): void {
  ...
  ctx.tools.set('provision_auth', {
    ...
    execute: async (input) => {
      ...
      lines.push('  - Request body is a type boundary: never pass raw `await request.json()`, `req.body`, or FormData values directly into `db.insert(...).values(...)` / `db.update(...).set(...)`. Browser number inputs arrive as strings; for every Drizzle `integer()` / `real()` column, use `z.coerce.number().finite()` or explicit `Number(...)` validation before writing.')
      lines.push('')
