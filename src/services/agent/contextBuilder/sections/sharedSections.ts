...
export function sharedDoingTasksCore(actor: 'developer' | 'user', scopeDescription: string): string {
 ...
 - Continuing when you already have what you need is a defect, not thoroughness: re-reading a file you have read, re-running a search you already ran, or re-verifying what already passed. When your investigation stops producing new information, act on what you have or ask the developer. (The REQUIRED independent Verify pass on non-trivial changes is not "re-verifying" — it is part of "done", not an extra.)`
}
