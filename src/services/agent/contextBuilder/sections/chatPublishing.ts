...
**Pre-write mechanical check (before any \`write_file\` / \`edit_file\` on \`db.ts\`)** — keep the local/prod boundary intact:

  1. The local env may contain \`DATABASE_URL=file:./dev.db\`; if absent, the code must still default to \`file:./dev.db\`.
  2. Do **not** ask for, write, or require \`TMDB_URL\` / \`TMDB_TOKEN\` during local dev. Those are deploy-time values.
  3. Generate schema and migrations now; Publish will translate/apply those migrations to Turso.
...
**Step 3** — Generate \`server/db.ts\` with the dev/prod connection switch. Local dev uses a SQLite file (\`file:./dev.db\`) via \`drizzle-orm/libsql/node\`; production uses \`drizzle-orm/sqlite-proxy\` pointing at deploy-injected \`process.env.TMDB_URL\` with \`Authorization: Bearer \${TMDB_TOKEN}\`. If you find yourself writing \`libsql://...turso.io\`, \`TMDB_URL\`, or \`TMDB_TOKEN\` into local project files/env during scaffolding, **stop** — that's the leak path that bypasses the worker boundary.
...
**Request-body type boundary — production-critical**: never pass raw request JSON/FormData into Drizzle writes. Browser form values arrive as strings even for \`<input type="number">\`, and local SQLite may coerce \`"1.3"\` while the production sqlite-proxy rejects it as \`invalid type: string, expected f64\`. For every route that writes to \`integer()\` or \`real()\` columns, validate and coerce at the backend boundary with \`z.coerce.number().finite()\` (or explicit \`Number(...)\` plus a 400 on \`NaN\`). Never write \`db.insert(table).values(body)\`, \`db.update(table).set(body)\`, or \`{ price: body.price }\` for numeric columns; write a typed object whose numeric fields are already numbers.

**Step 4.5 (only when the project handles file uploads)** — Call \`provision_files()\`. It mints an app-scoped \`TM_FILES_TOKEN\` and writes \`TM_FILES_URL\` + \`TM_FILES_TOKEN\` + \`TM_FILES_PUBLIC_BASE\` to \`.env\`. Then generate the helper at \`backend/src/files.ts\` (or \`server/src/files.ts\` if the project uses that layout — the bundle detects both) with the recipe from publish-backend §9.5. Storage layout: \`{slug}/_files/{key}\` on the platform's R2 bucket; public read URL is \`https://{slug}.toquemedia.net/_files/{key}\` — same origin as the frontend, zero CORS, no Worker hop on read, and media playback supports HTTP Range / 206. **DB columns store the returned \`publicUrl\` string, never the bytes.** For videos/audio, use the multipart/stream helper from §9.5 and never \`multer.memoryStorage()\`. Base64-in-DB is the anti-pattern: 33% row bloat, no CDN, kills query latency, fails at multi-MB. The pre-deploy lint catches the common Drizzle shape but the discipline is "never put bytes in TMDB columns" — not "rely on the lint".

**SQL:** For performance on large tables, add explicit indexes via Drizzle schema (\`index('name').on(table.col1, table.col2)\`) — they become \`CREATE INDEX\` in the generated migration. No platform-side index manifest, no FAILED_PRECONDITION runtime errors.
...
3. **Raw request bodies never go directly into Drizzle writes.** Every \`request.json()\` / \`FormData\` payload is validated first; numeric Drizzle columns receive real JS numbers, not string values from inputs. Use \`z.coerce.number().finite()\` at the route boundary.
4. **\`Dockerfile\` at the project root is generated in the SAME scaffold turn as the backend.** Without it, Publish ships only the frontend and the backend never comes online — the silent-failure mode that costs the most user trust. Treat the Dockerfile as load-bearing as \`server/index.ts\` itself. **\`migrations/\` folder is also copied into the image** so the worker can apply pending migrations against the app's TMDB at deploy time.
