...
export function usePromptBar() {
  ...
  const runAgentForPrompt = useCallback(async (
    ...
    // for image analysis, GLM-5.2 as primary) receive an OpenAI-compatible
    ...
    // plan: paid plans use GLM-5.2 (primary) + Qwen 3.6 Plus (image analysis),
    ...
    if (supportsAttachments && display.attachments.some(a => a.type === 'image')) {
      ...
      if (parts) {
        ...
        // active model (MiMo V2.5 Pro, GLM-5.2 → supportsAttachments=false)
