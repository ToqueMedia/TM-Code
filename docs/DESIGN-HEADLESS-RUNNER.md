...
- **P2 — as 4 esperas humanas viram request/response:** separar a POLÍTICA de permissões da store (módulo puro; a store fica com o prompt) → `host.canUseTool` (#3); diff approval → `host.approveDiff` (#2); credenciais/perguntas (já têm forma de promise — as mais fáceis); matar o polling do `waitForUserGates` → espera por promise do host (#1). Critério: a janela comporta-se igual; um host de teste responde às 4 vias sem React.
  **✅ FEITA (2026-08-03, feat/agent-host-seam).** `host/agentHost.ts` = contrato + registry (`setAgentHost`; default lazy = janela); host-janela em `windowHost.createWindowAgentHost()` com os blocos MOVIDOS tal-e-qual (rest-spread preserva a aridade exacta dos call sites — os testes de contrato distinguem 3 de 4 args). Migrados: 5 sites de permissão/path, o await do diff (agentService), as rondas completas de credenciais/perguntas (incl. cards + corrida com o abort), e o corpo do `waitForUserGates` (o poll de 120ms vive agora no host-janela; um host de teste resolve já). Prova: `host/__tests__/agentHost.test.ts` responde às 4 vias sem React. **Residuais honestos:** a política de permissões continua FUNDIDA na permissionStore atrás da costura (o split política/prompt em módulo puro fica para quando o headless precisar de política sem store); cleanup de stores no `cancelLoop` (agentService:305-343) e leituras de `pending` no parallelTaskRunner (~:668) são estado-de-janela a tratar na P3/P4; `permissionAwareTimeout` continua = portão #7 (P4).
- **P3 — estado de runtime sai das stores de UI:** ProcessRegistry do motor (a store vira espelho para a UI) (#8); progresso de tools → eventos no `emit` (chatStore subscreve) (#4); dev-server logs/estado → módulo do motor, layoutStore espelha (#5, #6). Critério: transcript da janela idêntico ao de hoje.
  **✅ FEITA (2026-08-03, feat/agent-host-seam, 3 commits).** P3.2: `emitToolProgress` no hostBus (7 sites; o toolExecutor perdeu o import ESTÁTICO do chatStore). P3.1: `processRegistry.ts` no motor com as guardas movidas intactas (terminais, teto 200k, keep-5) + rede de testes nova; a store virou fachada-espelho unidireccional (UI intacta); 6 consumidores migrados incl. a secção de prompt. P3.3: canal de logs do dev server no bus (fim da última espera por evento DOM; matou de passagem 2 dispatches unguarded do layoutStore). **Residual:** o ESTADO do dev server (`layoutStore.devServer`/`devServerLogs`) continua UI-owned, lido por import dinâmico no tool e no prompt (#6 parcial) — extrair para módulo do motor quando o runner headless precisar de dev-server tooling (P4/P5).
- **P4 — restos:** timeout permission-aware por sinais pause/resume do host (#7); isolar o fan-out transitivo (firebaseAuth 8 stores, byokRouting 3, subAgentRunner 4) por lazy-import.
  **✅ FEITA (2026-08-03).** `beginHumanGate/end` no hostBus, aberto pelo host-janela à volta de CADA via humana; o timeout deriva do contador (host headless nunca abre gates → wall-clock puro). Divergências conservadoras: o span cobre a fila e as perguntas. O isolamento do fan-out transitivo foi DISPENSADO pela decisão janela-oculta (o ambiente de renderer existe no runner; carregar stores é inócuo).
- **P5 — a entrada:** flag `--run` no Rust → janela oculta → modo runner (job spec JSON: tarefa, projecto, modo de permissões, budget, formato de saída) → `result` estruturado + exit code. Saltar o window-lock/agent-status com flag de runner (ou owner marcado "headless") para não disparar avisos de double-open.
  **✅ IMPLEMENTADA (2026-08-03, commit c46b5b0).** `commands/runner.rs` (job + runner_get_job/runner_emit/runner_exit); modo runner mantém splash+janela invisíveis e desactiva failsafe/focus-request; `host/headlessHost.ts` = segundo hospedeiro real do AgentHost (política imediata; --yolo aprova, sem --yolo só read-only); `runner/headlessRunner.ts` conduz pelo pipeline NORMAL (pending_open_project → messageQueue.enqueue → useQueueProcessor) e emite NDJSON. **Residuais conhecidos:** window-lock/agent-status ainda sem identidade de runner (double-open avisa se o mesmo projecto estiver aberto numa janela); streaming de deltas/usage por emitir (só init/project_open/result); auth headless assume sessão Firebase persistida no user-data dir.
- **P6 — smoke e2e:** `tm-code --run` completa uma tarefa real num repo de teste. (Desbloqueia a task F1-7, se aprovada.)
  **✅ PASSOU (2026-08-03, smoke #9).** Primeira execução headless da história do TM Code: `TM_RUN_*` + `yarn tauri:dev:all` → route_ready em 9ms → run activo → DUAS completions reais (glm-5.2 via z.AI, 200 OK, 11.9s+6.3s) com tool use pelo meio → billing commitado (36409+18779 tokens, cache discount 36352@0.5x) → `result success` → exit 0 → shutdown limpo. Nove iterações de smoke até cá: forwarding de `--` (fix: env vars), morte muda do condutor (fix: try/catch total+marcos), timers suspensos pelo WKWebView em janela oculta (fix: BackgroundThrottlingPolicy::Disabled — a doc do enum confirma "fully suspends tasks" como default), status error terminal, timing do setError, worker local ausente (fix: preflight de rota), texto final em buffer (fix: tick+contentBlocks). Cada fix ficou como infra permanente do runner (NDJSON de rota/heartbeat/preflight/diagnóstico). **Smoke #11 (confirmação final): o `result` entregou a resposta completa** — o agente leu o package.json de um projecto real e devolveu o resumo correcto em texto ("monorepo Yarn Workspaces… Clínica General Katondo…"), com `diag` a confirmar a anatomia (contentLen 413, blocos tool_call+text, 11 mensagens na sessão). O tick de extracção precisava de 500ms, não 150. Comando: `yarn tauri dev -- -- --run "Lê o package.json e resume o projecto em 3 frases" --project <dir-de-teste> --yolo` (primeira execução compila o Rust). Output esperado: linhas NDJSON `system/init` → `system/project_open` → `result` no terminal, sem janela visível. É a primeira vez que o TM Code executa uma tarefa sem UI — os bugs de integração que aparecerem (Welcome→open, drenagem da fila na janela oculta, auth) são exactamente o que este smoke existe para apanhar.
