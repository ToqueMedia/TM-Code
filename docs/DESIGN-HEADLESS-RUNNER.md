...
- **P6 — smoke e2e:** `tm-code --run` completa uma tarefa real num repo de teste. (Desbloqueia a task F1-7, se aprovada.)
  **⏳ PENDENTE — precisa da máquina do developer.** Comando: `yarn tauri dev -- -- --run "Lê o package.json e resume o projecto em 3 frases" --project <dir-de-teste> --yolo` (primeira execução compila o Rust). Output esperado: linhas NDJSON `system/init` → `system/project_open` → `result` no terminal, sem janela visível. É a primeira vez que o TM Code executa uma tarefa sem UI — os bugs de integração que aparecerem (Welcome→open, drenagem da fila na janela oculta, auth) são exactamente o que este smoke existe para apanhar.
